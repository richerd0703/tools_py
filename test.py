...
def copyFiles(path, path2, obj_path):
    count = 0
    name_list = [x for x in os.listdir(path2) if x.endswith(".png")]
    for i in range(len(name_list)):
        name_list[i] = name_list[i].split('.')[0]
    l = len(name_list)
...
if __name__ == '__main__':
    ...
    path = r'G:\test\xihu+beijing\hz\img1\png\split'  # 需要找的文件夹
    path2 = r'G:\test\xihu+beijing\hz\521x512\lab1'  # 已经确定的文件夹
    obj_path = r'G:\test\xihu+beijing\hz\521x512\img1'  # 保存路径
